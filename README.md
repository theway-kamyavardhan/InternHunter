...
1. **Interactive Setup Wizard**: Beautiful Claude-Code style CLI wizard that detects your vault and safely manages API keys on first run.
2. **Obsidian Self-Learning Engine**: Scans past applications in your vault and uses an LLM to learn what subject lines and personal touches get replies, permanently improving future cold emails.
3. **AI Confidence Scorer**: Scores every scraped job 0-100 based on *your* resume. Only applies to high-signal roles (≥75).
4. **Company Intelligence Injection**: Pulls recent news, funding, or product launches for "Dream Companies" and weaves 1-2 hyper-relevant sentences into the cold email.
5. **Obsidian-Native Kanban Tracking**: Auto-creates beautiful Markdown notes with YAML frontmatter. Real-time Dataview analytics for Win-Rate, Response Time, and Pipeline Health.
6. **Telegram Human-Guidance Mode**: If the "Personal Touch Score" is low, or it's a Dream Company, it pauses the pipeline and sends the draft to Telegram. You reply `APPROVE`, `EDIT: ...`, `SKIP`, or `REWRITE`.
7. **Reply Intelligence**: Connects to your inbox via IMAP. If a company replies "not hiring", it auto-moves them to "Rejected/Ghosted". If they want an interview, it alerts you!
...
```bash
git clone https://github.com/theway-kamyavardhan/InternHunter.git
cd intern-hunter-ai
./setup.sh
```
...
```bash
# Launch the interactive setup wizard (runs automatically on first use)
intern-hunter setup

# Force the self-learning engine to analyze your Obsidian vault
intern-hunter learn
...
### 1. Tailored PDF Generation
![ATS-Friendly PDF Generated by WeasyPrint](assets/pdf_example.png)

### 2. Obsidian Kanban Board
![Obsidian Kanban Board with Dataview Analytics](assets/kanban.png)

### 3. Telegram Human Guidance
![Telegram Bot asking for approval](assets/telegram.png)
